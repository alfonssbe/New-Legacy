                  ...
                  {row.getVisibleCells().map((cell, indexcol) => (
                    indexcol === indexPreview ? (
                      /* @ts-ignore */<TableCell key={cell.id}><div style={{ display: 'flex' }}> 
                        {data[Number(row.id)].productImageUrl[0]!=''?<Image src={data[Number(row.id)].productImageUrl[0]} alt={data[Number(row.id)].name} width={200} height={200} className="w-56 h-fit"/>:<FileQuestion size={30}/>}
                      </div>
                      </TableCell>
                    ) : indexcol === indexPreviewFeatured ? (
                      /* @ts-ignore */<TableCell key={cell.id}><div style={{ display: 'flex' }}>
                        {data[Number(row.id)].value!=''?<Image src={data[Number(row.id)].value} alt={data[Number(row.id)].name} width={200} height={200} className="w-56 h-fit"/>:<FileQuestion size={30}/>}
                      </div>
                      </TableCell>
